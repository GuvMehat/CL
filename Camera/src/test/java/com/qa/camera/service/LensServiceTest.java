...
public class LensServiceTest {
	...
	void updateTest() throws Exception { 
		final Long id = 1L;
		final Lens lens1 = new Lens(1L,2,1L,2, "tameron");
		final Lens lens2 = new Lens(1L,2,1L,2, "tameron");
		final LensDTO lensDTO = new LensDTO();

		Mockito.when(this.repo.findById(id)).thenReturn(Optional.of(lens1));
		Mockito.when(this.repo.save(lens2)).thenReturn(lens2);

		assertEquals(lensDTO, this.service.update(id, lens2));

		Mockito.verify(this.repo, Mockito.times(1)).findById(Mockito.anyLong());
		Mockito.verify(this.repo, Mockito.times(1)).save(lens2);
	}
